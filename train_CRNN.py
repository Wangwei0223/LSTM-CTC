...
learning_rate = 0.001
MOMENTUM = 0.9
n_hidden = 256  # The hidden units in RNN
...
img = tf.Variable(tf.random_normal([batch_size, 32, 100, 1]))  # 32*100

x = tf.placeholder(tf.float32, [None, 32, 100, 1])  # as the input
...
# SparseTensor required by CTC layer.
targets = tf.sparse_placeholder(tf.int32)  # as the label

seq_len = tf.placeholder(tf.int32, [None])  # 1d array of size [batch_size] 
    ...
    with tf.Session() as sess:
        ...
        for curr_epoch in range(250):
            ...
            for batch in range(num_batches_per_epoch):
                ...
                sparse_targets = model.sparse_tuple_from_label(train_targets[indexes])   # get the sparse_tragets
...
def test_network():
    with tf.Session() as sess:
        ...
        path = os.path.abspath('..') + "/Model_CRNN 2/CRNN_MNIST_Image.ckpt-4" # Change your save path
